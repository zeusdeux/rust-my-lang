...
impl fmt::Display for ReplError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            ReplError::UnknownInputMode => write!(
                f,
                "Repl: Unknown input mode. Only multiline and singleline are supported."
            ),
            ReplError::InvalidEvalResult(s) => write!(f, "Invalid eval result received -> {}", s),
        }
    }
}
...
fn main() -> Result<(), ReplError> {
    ...
    println!("{}", ReplError::InvalidEvalResult("omg wot".to_string()));
    println!("{}", ReplError::UnknownInputMode);
